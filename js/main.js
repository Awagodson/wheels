...
(function ($) {
	...
	$(".qtybutton").on("click", function () {
		if ($('.cart-plus-minus').attr("data-id") == 'no-api') {
			var $button = $(this);
			var oldValue = $button.parent().find("input[type=text]").val();
			if ($button.text() == "+") {
				// Don't allow incrementing above four
			...
			var $button = $(this);
			var oldValue = $button.parent().find("#quantity_val").val();
			var id = $button.parent().find("#item_id").val()
			if ($button.text() == "+") {
				if(confirm("Do you really want to increase the quantity?")) {
					var newVal = parseFloat(oldValue) + 1;
					$.post("/api/handle-cart/",
					{
						id: id,
						type: '1',
				...
				if(confirm("Do you really want to decrease the quantity?")) {
